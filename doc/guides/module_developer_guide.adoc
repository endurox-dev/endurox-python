...
Enduro/X Python mode is based on native code binding. Because
Python loads shared libraries with *RTLD_LOCAL* flag, the consequent
shared libraries loaded by Enduro/X code (XA libs) do not see the
...
=== On MacOS pip3 install of source or binary package might fail

If performing "pip3 install . --user" of the source or binary wheel
package and error such as is generated:
...
However, another option is to follow the installation process of the Enduro/X for Python3
by using source package and "python3 ./setup.py install --user" command, which 
with default configuration does succeed (no above error generated).
...
shall be enough to start to dig into module development details. Also, the document can
