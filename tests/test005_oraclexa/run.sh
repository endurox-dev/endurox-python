...
#
# @(#) Test 005 - Oracle XA distributed transactions, with cx-oracle
#
...
################################################################################
echo "Running Oracle XA transactions"
################################################################################

NDRX_CCTAG=ORA python3 -m unittest client.py

RET=$?

if [ $RET != 0 ]; then
    echo "client.py failed"
    go_out -1
fi
