...
xmemck -v10 -d10 -s20 -t95 -m "0myWI5nu|unittest" 2>$XMEMCK_LOG 1>$XMEMCK_OUT &
MEMCK_PID=$!
...
################################################################################
echo "Running VIEW not found"
################################################################################

# prints error...
NDRX_CCTAG=low python3 -m unittest client-view-missing.py

RET=$?

if [ $RET != 0 ]; then
    echo "client-view-missing.py failed"
    go_out -1
fi
...
################################################################################
echo "UbfDict checks, slice"
################################################################################

NDRX_CCTAG=low python3 -m unittest client-ubf-ubfdict-silce.py 

RET=$?

if [ $RET != 0 ]; then
    echo "client-ubf-ubfdict-silce.py failed"
    go_out -1
fi
